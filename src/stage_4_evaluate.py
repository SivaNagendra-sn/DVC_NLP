import argparse
import os
import shutil
from tqdm import tqdm
from utils.common import read_yaml, save_json
import logging
import numpy as np
import joblib
import sklearn.metrics as metrics
import json, math


logging.basicConfig(
    filename=os.path.join("logs", 'running_logs.log'), 
    level=logging.INFO, 
    format="[%(asctime)s: %(levelname)s: %(module)s]: %(message)s",
    filemode="a"
    )

STAGE = "Evaluate"


def main(config_path, params_path):
    config = read_yaml(config_path)
    params = read_yaml(params_path)

# Load the trained model from model path

    artifacts = config["artifacts"]
    model_dir_path = os.path.join(artifacts["ARTIFACTS_DIR"], artifacts["MODEL_DIR"])
    # create_directories([model_dir_path])
    model_path = os.path.join(model_dir_path, artifacts["MODEL_NAME"])

    load_model = joblib.load(model_path)

# Load & transoform the test data

    featurized_data_dir_path = os.path.join(artifacts["ARTIFACTS_DIR"],artifacts["FEATURIZED_DATA"])
    featurized_test_data_path = os.path.join(featurized_data_dir_path, artifacts["FEATURIZED_OUT_TEST"])

    test_matrix = joblib.load(featurized_test_data_path)

    test_labels = np.squeeze(test_matrix[:, 1].toarray())
    X_test = test_matrix[:, 2:]

    logging.info(f"Input matrix shape : {test_matrix.shape}")
    logging.info(f"X matrix shape : {X_test.shape}")
    logging.info(f"labels matrix shape : {test_labels.shape}")

# Predict with the test data 
     
    predicted_val_by_class = load_model.predict_proba(X_test)
    predictions = predicted_val_by_class[:, 1]

    print(predictions)

    msg = f"Evaluation completed for the test dataset"
    logging.info(msg)

# Scores and log the metrics for predicted values

    PRC_json_path = config["plots"]["PRC"]
    ROC_json_path = config["plots"]["ROC"]
    scores_json_path = config["metrics"]["SCORES"]

    precision, recall, prc_threshold = metrics.precision_recall_curve(test_labels, predictions)

    nth_point = math.ceil(len(prc_threshold)/1000)
    prc_points = list(zip(precision, recall, prc_threshold))[::nth_point]
    prc_data = {
        "prc": [
            {"precision": p, "recall" : r, "threshold": t}
            for p, r, t in prc_points
        ]
    }

    save_json(PRC_json_path, prc_data)

    fpr, tpr, roc_threshold = metrics.roc_curve(test_labels, predictions)

    roc_data = {
        "roc":[
            {"fpr": fp, "tpr": tp, "threshold":t}
            for fp, tp, t in zip(fpr, tpr, roc_threshold)
        ]
    }
    save_json(ROC_json_path, roc_data)

    avg_precision = metrics.average_precision_score(test_labels, predictions)
    roc_auc = metrics.roc_auc_score(test_labels, predictions)

    scores = {
        "avg_precision": avg_precision,
        "roc_auc": roc_auc
    }
    save_json(scores_json_path, scores)


if __name__ == '__main__':
    args = argparse.ArgumentParser()
    args.add_argument("--config", "-c", default="configs/config.yaml")
    args.add_argument("--params", "-p", default="params.yaml")
    parsed_args = args.parse_args()

    try:
        logging.info("\n********************")
        logging.info(f">>>>> stage {STAGE} started <<<<<")
        main(config_path=parsed_args.config, params_path = parsed_args.params)
        logging.info(f">>>>> stage {STAGE} completed! <<<<<\n")
    except Exception as e:
        logging.exception(e)
        raise e